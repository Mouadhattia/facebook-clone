...
const HomeHeader = ({ user, selected }) => {
  ...
  useEffect(() => {
    ...
    if (users !== undefined) {
      const finaUsers = users.filter((user) => {
        return user.name.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1;

      });
      setFilteredUsers(finaUsers);
    }
  }, [searchTerm]);
  ...
  return (
    <div className="homeHeader">
      <div className="homeHeaderLogoAndSearch">
        <Link to="/">
          <img
            src="https://www.meilleure-innovation.com/wp-content/uploads/2021/11/1200px-facebook-logo-2019-1024x1024.png"
            alt="logo"
            className="homeHeader__logo"
          />
        </Link>
        <div className="homeHeader__searchBack" onClick={collapesNavbar}>
          <i className="searchBackIcon" />
        </div>
        <div className="homeHeader__search" onClick={expanNavbar}>
          ...
          <ul id="list">
            {users !== undefined &&
              filteredUsers.map((user1) => (
               ...
               <Fragment>
                 <li className="recent-search"><p className="recent">No recent research</p></li>
                <li>
                  
                  <a href= {`/${user1.name}/${user1.uid}`} onClick={collapesNavbar}>
